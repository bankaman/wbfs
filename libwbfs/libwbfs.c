...
static u32 find_free_block(wbfs_t *p, u32 from_block)
{

    u32 start_i = from_block / 32;
    u32 start_j = from_block - start_i * 32;

    u32 i, j;
    for (i = start_i; i < p->n_wbfs_sec / (8 * 4); i++)
    {
        u32 v = wbfs_ntohl(p->freeblks[i]);
        if (v != 0)
        {
            for (j = start_j; j < 32; j++)
            {
                if (v & (1 << j))
                {
                    return (i * 32) + j + 1;
...
static u32 find_non_free_block(wbfs_t *p, u32 from_block)
{
...
void mark_badblock(wbfs_t *p, u32 block_id)
{
    fprintf(stderr, "\n mark badblock #(%u)\n", block_id);
...
int compareBuffer(u8 *buffer, u32 size, u8 value)
{
...
void forEachFreeBlock(wbfs_t *p, progress_callback_t spinner, u8 *buf, void (*callback)(wbfs_t *p, u32 blockId, u8 *buf, void *param[]), void *param[])
{
    u32 curBlock = 0;
    u32 i = 0;
    u32 freeBlockCnt = wbfs_count_usedblocks(p);

    if (spinner)
    {
        spinner(0, freeBlockCnt);
    }

    while (curBlock != ~0U)
    {
        curBlock = find_free_block(p, curBlock);
        ...
        callback(p, curBlock, buf, param);

        i++;
        if (spinner)
        {
            spinner(i, freeBlockCnt);
        }
    }
}

void writeBufferToBlock(wbfs_t *p, u32 curBlock, u8 *buf, void *param[])
{
    u32 blockSize = p->wbfs_sec_sz / p->hd_sec_sz;
    u32 lba = p->part_lba + curBlock * blockSize;
    int err = p->write_hdsector(p->callback_data, lba, blockSize, buf);
    if (err != 0)
    {
        mark_badblock(p, curBlock);
    }
}

void invertBlock(wbfs_t *p, u32 curBlock, u8 *buf, void *param[])
{
    u8 testValue = *(u8 *)param[0];
    u8 *hadError = (u8 *)param[1];

    u32 blockSize = p->wbfs_sec_sz / p->hd_sec_sz;
    u32 lba = p->part_lba + curBlock * blockSize;

    int err = p->read_hdsector(p->callback_data, lba, blockSize, buf);
    if (err != 0)
    {
        mark_badblock(p, curBlock);
        *hadError = 1;
    ...
    if (!compareBuffer(buf, p->wbfs_sec_sz, testValue))
    {
        mark_badblock(p, curBlock);
        *hadError = 1;
        return;
    ...
    for (u32 k = 0; k < p->wbfs_sec_sz; k++)
    {
        buf[k] = ~buf[k];
    }

    err = p->write_hdsector(p->callback_data, lba, blockSize, buf);
    if (err != 0)
    {
        mark_badblock(p, curBlock);
        *hadError = 1;
...
void checkValue(wbfs_t *p, u32 curBlock, u8 *buf, void *param[])
{
    u8 testValue = *(u8 *)param[0];
    u8 *hadError = (u8 *)param[1];
...
void writeBlockId(wbfs_t *p, u32 curBlock, u8 *buf, void *param[])
{
    u8 blockId[2] = {curBlock & 0xff, (curBlock >> 8) & 0xff};
    ...
    writeBufferToBlock(p, curBlock, buf, NULL);
}

void compareBlockId(wbfs_t *p, u32 curBlock, u8 *buf, void *param[])
{
    u8 blockId[2] = {curBlock & 0xff, (curBlock >> 8) & 0xff};
    u8 *hadError = (u8 *)param[0];
...
void readWriteValueTest(wbfs_t *p, progress_callback_t spinner, u8 *buf, u8 value, u8 *hadError)
{
    dropCache();
    u32 freeBlockCnt = wbfs_count_usedblocks(p);
    wbfs_memset(buf, value, p->wbfs_sec_sz);
    fprintf(stderr, "Filling free blocks with value 0x%x (%u)\n", value, freeBlockCnt);
    forEachFreeBlock(p, spinner, buf, &writeBufferToBlock, NULL);

    fprintf(stderr, "\nSaving...\n");
    wbfs_sync(p);

    dropCache();
    freeBlockCnt = wbfs_count_usedblocks(p);
    fprintf(stderr, "\nCheck value 0x%x (%u)\n", value, freeBlockCnt);
    forEachFreeBlock(p, spinner, buf, &checkValue, (void *[]){&value, &hadError});
...
repeat:
    ...
    u8 testValues[] = {0x55, 0xAA, 0x00, 0xFF};

    for(int i=0; i< sizeof(testValues);i++){
        readWriteValueTest(p, spinner, copy_buffer, testValues[i], &hadError);    
    }

    dropCache();
    freeBlockCnt = wbfs_count_usedblocks(p);
    fprintf(stderr, "\nFilling with block number (%u)\n", freeBlockCnt);
    forEachFreeBlock(p, spinner, copy_buffer, &writeBlockId, (void *[]){&hadError});
    ...
    dropCache();
    freeBlockCnt = wbfs_count_usedblocks(p);
    fprintf(stderr, "\nCompare with block number (%u)\n", freeBlockCnt);
    forEachFreeBlock(p, spinner, copy_buffer, &compareBlockId, (void *[]){&hadError});
...
void wbfs_list_filled_blocks(wbfs_t *p)
{
    ...
    while (blockId != endId)
    {
        blockId = find_non_free_block(p, blockId);
        if (blockId == endId)
        {
            break;
        }
        u32 lba = p->part_lba + blockId * blockSize;
        fprintf(stderr, "# %u (offset: %u, size: %u)\n", blockId, lba, p->wbfs_sec_sz);
    }
}
